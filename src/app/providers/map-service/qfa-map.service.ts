...
export class QfaMapService {
  ...
  public typeLayers: Record<ObservationType, LayerGroup>;
  public overlayMaps: {
  ...
  constructor(
    ...
    this.typeLayers = {} as any;
    Object.keys(ObservationSource).forEach(source => this.sourceLayers[source] = new LayerGroup());
    Object.keys(ObservationType).forEach(type => this.typeLayers[type] = new LayerGroup());
    this.prepear();
  }
  ...
  initMaps(el: HTMLElement, onObservationClick: (o: GenericObservation) => void) {
    ...
    Object.values(this.typeLayers).forEach(aLayer => {
