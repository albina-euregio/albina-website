...
export class QuickReportModel {
	...
	removeSnowCondition(snowCondition) {
		let index = this.snowConditions.indexOf(snowCondition);
		if (index > -1)
	...
	removeRode(rode) {
		let index = this.rode.indexOf(rode);
		if (index > -1)
	...
	removeAvoided(avoided) {
		let index = this.avoided.indexOf(avoided);
		if (index > -1)
	...
	removeAvalancheProblem(avalancheProblem) {
		let index = this.avalancheProblems.indexOf(avalancheProblem);
		if (index > -1)
	...
	static createFromJson(json, id?) {
		let quickReport = new QuickReportModel(json.showUsername);
		...
		if (json.snowConditions) {
			let snowConditions = new Array<string>();
