...
class SubscribeEmailDialog extends React.Component {
  ...
  handleChangeRegion = newRegions => {
    ...
    const equal =
      newRegions.length == this.state.regions.length &&
            ...
            <ul className='list-inline list-buttongroup'>
              <li>
                <button
                  ...
                  className={
                    'pure-button' +
                      (this.state.regions.length == 1 ? ' inverse' : '')
                  }
                  onClick={() => {
                    this.handleChangeRegion(
                      Object.keys(window['appStore'].regions)
                    )
                  }}>
              ...
              <li>
                <span className='buttongroup-boolean'>
                  {this.props.intl.formatMessage({
                    id: 'dialog:subscribe-email:region:or'
                  })}
                </span>
              </li>
              <li>
                <ProvinceFilter
                  className={
                    this.state.regions.length == 1 ? 'selectric-changed' : ''
                  }
                  all={this.props.intl.formatMessage({
                    id: 'dialog:subscribe-email:region-select:button'
                  })}
                  handleChange={r => this.handleChangeRegion(r)}
                  value={
                    this.state.regions.length == 1 ? this.state.regions[0] : ''
                  }
                />
              </li>
            </ul>
