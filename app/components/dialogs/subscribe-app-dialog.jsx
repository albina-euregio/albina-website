  ...
  render() {
    ...
    apps.forEach((a) => {
      downloads[a.id] = Object.keys(a.url).map((t) => (
        <li key={t}>
          <a className="pure-button tooltip"
            href={a.url[t]}
            target="_blank"
            title={this.props.intl.formatMessage({id: 'dialog:subscribe-app:' + t + ':hover'})} >
            {this.props.intl.formatMessage({id: 'dialog:subscribe-app:' + t + ':button'})}
          </a>
        </li>
      ));
    })
...

        {
          apps.map((a) =>
            <div className="app-dl" key={a.id}>
              ...
              <h2 className="subheader"><FormattedHTMLMessage id={'dialog:subscribe-app:' + a.id + ':title'} /></h2>
              <p className="small"><FormattedHTMLMessage id={'dialog:subscribe-app:' + a.id + ':text'} /></p>
