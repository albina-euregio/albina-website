...
L.Control.Geonames = L.Control.extend({
  ...
  options: {
    ...
    geonamesPostalCodesSearch:
      'https://secure.geonames.org/postalCodeSearchJSON', // override this if using a proxy to get connection to geonames
    ...
    clearOnPopupClose: false, // clear search input and results on popup close
  ...
  _onPopupClosed: function() {
    if (this.options.clearOnPopupClose) {
      this._close()
      this._input.value = ''
      this._hasResults = false
      this._resultsList.innerHTML = ''
  ...
  _search: function(event) {
    ...
    if (
      this.options.enablePostalCodes &&
      this.options.postalCodesRegex.test(query)
    ) {
    ...
    for (param in this.options.adminCodes) {
      ...
      searchParams[param] =
        typeof paramValue == 'function' ? paramValue() : paramValue
    }

    // Add bbox to query
    var bbox =
      typeof this.options.bbox == 'function'
        ? this.options.bbox()
        : this.options.bbox
    for (i in BBOX) {
    ...
    var url =
      apiURL +
      '?' +
      this._objToQuery(coreParams) +
      '&' +
      this._objToQuery(searchParams) +
      extraQueryParams
  ...
  _processResponse: function(response) {
    ...
    if (jsonResponse.length > 0) {
      ...
      L.DomUtil.addClass(this._resultsList, 'noResults')
      li = L.DomUtil.create('li', '', this._resultsList)
      li.innerText = this.options.noResultsFoundTitle
    }
  },
