...
function getDemoById(id) {
  return demos.filter(function(a) {
    return a.id === id;
  })[0];
}
...
function keyboardNavigation(e) {
  ...
  switch (e.keyCode) {
    case 38:
      ...
      while (
        prevEl &&
        !prevEl.classList.contains("demo") &&
        prevEl.parentNode.previousElementSibling
      ) {
        prevEl = prevEl.parentNode.previousElementSibling.lastElementChild;
      }
      if (prevEl && prevEl.classList.contains("demo"))
        getDemoById(prevEl.id).highlight(e, true);
      break;
    case 40:
      var nextEl = activeDemoEl.nextElementSibling;
      if (!nextEl && activeDemoEl.parentNode.nextElementSibling) {
        nextEl = activeDemoEl.parentNode.nextElementSibling.querySelector(
          ".demo"
        );
      }
      if (nextEl && nextEl.classList.contains("demo"))
        getDemoById(nextEl.id).highlight(e, true);
      break;
  }
}
