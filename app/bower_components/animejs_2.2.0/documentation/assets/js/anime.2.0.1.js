...
(function(root, factory) {
  ...
  function getUnit(val) {
    const split = /([\+\-]?[0-9#\.]+)(%|px|pt|em|rem|in|cm|mm|ex|pc|vw|vh|deg|rad|turn)?/.exec(
      val
    );
  ...
  function validateValue(val, unit) {
    ...
    const unitLess = originalUnit
      ? val.substr(0, arrayLength(val) - arrayLength(originalUnit))
      : val;
  ...
  function getInstanceTimings(type, animations, tweenSettings) {
    ...
    return arrayLength(animations)
      ? math.apply(Math, animations.map(anim => anim[type]))
      : tweenSettings[type];
  }
  ...
  function anime(params = {}) {
    ...
    function setAnimationsProgress(insTime) {
      ...
      while (i < arrayLength(animations)) {
        ...
        const tween =
          tweens.filter(t => insTime < t.end)[0] ||
          tweens[arrayLength(tweens) - 1];
        ...
        const progress = recomposeValue(
          tween.to.numbers.map((number, p) => {
  ...
  function timeline(params) {
    ...
    tl.add = function(instancesParams) {
      toArray(instancesParams).forEach(insParams => {
        ...
        insParams.offset = is.und(offset)
          ? tlDuration
          : getRelativeValue(offset, tlDuration);
